...
fn main() {
    App::new()
        ...
        .add_systems(
            Update,
            check_status.run_if(common_conditions::on_timer(Duration::from_millis(500))),
        )
...
#[derive(Component, Clone, Copy, Debug, MapEntities, Reflect)]
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let entity = commands
        .spawn((Planet, SceneRoot(planet_handle), planet_transform))
        .id();
...
fn check_status(mut text: Query<&mut Text, With<MoonStatus>>, moon_query: Query<&MoonOf>) {
